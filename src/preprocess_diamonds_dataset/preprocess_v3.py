"""
CHANGES from preprocess_v2.py:
- Lowered Pearson threshold to 0.70 to remove more moderately correlated predictors.
- Heatmap output renamed to `results/correlation_matrix_v3.png` for clarity.
- Cleaned dataset saved to `data/diamonds_v3.csv`.
"""
...
CLEANED_DATA_PATH = os.path.join("data", "diamonds_v3.csv")
HEATMAP_PATH = os.path.join(RESULT_PATH, "correlation_matrix_v3.png")
CORRELATION_THRESHOLD = 0.70
PROTECTED_COLUMNS = {"total_sales_price"}
...
def drop_highly_correlated(
    ...
    to_drop = [
        ...
        if column not in PROTECTED_COLUMNS and any(upper[column] > threshold)
    ]
    if to_drop:
